...
<script>
	...
	export default {
		...
		created() {
			axios.get("http://localhost:9527/api/getRankData").then((data) => {
		...
		methods: {
			...
			selectItem(item) {//点击跳转
				axios.get(`http://localhost:9527/api/getRankDetail/${item.topId}`).then(da=>{
					console.log(da.data)
                    let toplist;
					if(da.data[0]){
						toplist = da.data[0];
